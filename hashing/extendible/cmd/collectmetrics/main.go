...
type profileRecord struct {
	...
	Stats           struct {
		...
		GlobalDepth uint `json:"GlobalDepth"`
	} `json:"stats"`
}

func main() {
	var (
		sizesFlag      = flag.String("sizes", "10000,100000,1000000", "comma-separated workload sizes")
	...
	benchOutput, records, err := runBenchmarks(benchOutputPath, *sizesFlag)
	if err != nil {
	...
	profileRecords := make([]profileRecord, 0, len(sizes)*4)
	for _, operation := range []string{"insert", "update", "delete", "get"} {
		for _, size := range sizes {
			record, err := runProfile(operation, size, *bucketCapacity, profileDir)
			if err != nil {
...
func runBenchmarks(outputPath string, sizesStr string) (string, []benchmarkRecord, error) {
	cmd := exec.Command("go", "test", "-run", "^$", "-bench", "^BenchmarkTable", "-benchmem", ".")
...
func runProfile(operation string, size int, bucketCapacity int, outDir string) (profileRecord, error) {
	summaryPath := filepath.Join(outDir, fmt.Sprintf("%s_%d_summary.json", operation, size))
	cmd := exec.Command(
		"go", "run", "./cmd/profile",
		"-op", operation,
		"-size", strconv.Itoa(size),
		"-capacity", strconv.Itoa(bucketCapacity),
		"-out", outDir,
	)
	data, err := cmd.CombinedOutput()
	if err != nil {
...
func writeProfileCSV(path string, records []profileRecord) error {
	...
	if err := writer.Write([]string{"operation", "size", "elapsed_ns", "throughput_ops_s", "cpu_user_ns", "cpu_sys_ns", "total_alloc_diff", "alloc_count_total", "bucket_count", "global_depth"}); err != nil {
		return err
	}
	for _, record := range records {
		row := []string{
			...
			strconv.FormatUint(uint64(record.Stats.GlobalDepth), 10),
		}
		if err := writer.Write(row); err != nil {
			return err
		}
...
func parseBenchmarkOutput(output string) ([]benchmarkRecord, error) {
	pattern := regexp.MustCompile(`^BenchmarkTable(Insert|Update|Delete|Get)/size=(\d+)-\d+$`)
	scanner := bufio.NewScanner(strings.NewReader(output))
	records := make([]benchmarkRecord, 0)
	for scanner.Scan() {
		line := strings.TrimSpace(scanner.Text())
		if !strings.HasPrefix(line, "BenchmarkTable") {
			continue
		}
